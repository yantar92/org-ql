    ...
    (helm-make-source name 'helm-source-sync
      :filtered-candidate-transformer (lambda (candidate-markers _)
                                        (mapcar (lambda (candidate-marker)
                                                ...
                                                candidate-markers))
      :candidates (lambda ()
                    (let* ((query (let ((case-fold-search nil))
                                    (->> helm-pattern
                                      (replace-regexp-in-string "\\(?:^\\|[ \t]+\\):\\([^ \t]+\\)\\(?:[ \t]\\|$\\)" " tags:\\1 ")
                                      (replace-regexp-in-string "\\(?:^\\|[ \t]+\\)\\([^ \t]+\\)!\\(?:[ \t]\\|$\\)" " tags-local:\\1 ")
                                      (replace-regexp-in-string "[ \t]+A[ \t]" " tags-local:AREA ")
                                      (replace-regexp-in-string "[ \t]+P[ \t]" " tags-local:project ")
                                      (replace-regexp-in-string "[ \t]+a[ \t]" " tags-local:article ")
                                      (replace-regexp-in-string "[ \t]+b[ \t]" " tags-local:book ")
                                      (replace-regexp-in-string "[ \t]+S[ \t]" " todo:SOMEDAY ")
                                      (replace-regexp-in-string "[ \t]+N[ \t]" " todo:NEXT ")
                                      (replace-regexp-in-string "[ \t]+D[ \t]" " todo:DONE ")
                                      (replace-regexp-in-string "[ \t]+T[ \t]" " todo:TODO ")
                                      (replace-regexp-in-string "[ \t]+O[ \t]" " todo:DOING ")
                                      (replace-regexp-in-string "[ \t]+F[ \t]" " todo:FAILED ")
                                      (replace-regexp-in-string "[ \t]+W[ \t]" " todo:WAITING ")
                                      (replace-regexp-in-string "[ \t]+H[ \t]" " todo:HOLD ")
                                      (replace-regexp-in-string "[ \t]+C[ \t]" " todo:CANCELLED ")
                                      (replace-regexp-in-string "[ \t]+L[ \t]" " todo:TICKLER ")
                                      (replace-regexp-in-string "[ \t]+E[ \t]" " todo:REVIEW ")
                                      )))
                           (query (org-ql--query-string-to-sexp query)))
                      (when query
